...
# The fixed aligned BAM files do not have duplicates marked
# This script aims to mark the duplicates in reads (edited from the old script)
...
for bamfile in ${alignment_dir}/*_Aligned.sortedByCoord.out.bam; do
  ...
  # Extract base name
  sample_name=$(basename "$bamfile" _Aligned.sortedByCoord.out.bam)
